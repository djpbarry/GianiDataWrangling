...
pdf(paste("plots", "imaris_centroid_error_versus_snr.pdf", sep=.Platform$file.sep), pointsize=20);
boxplot(Centroid_Error~snr, data=allData, main="", xlab="SNR", ylab="Centroid Error (Microns)", notch=TRUE, col=(c("blue")));
...
write.csv(cellCountErrors, paste("outputs", "imaris_cell_count_errors.csv", sep=.Platform$file.sep));
...
pdf(paste("plots", "imaris_cell_count_errors.pdf", sep=.Platform$file.sep));
